...
export default function FormExcursionPlanning(props) {
  const { onCancel, programId, reservationId, planningId } = props;
  ...
  const fetchExcursions = async () => {
    ...
    setMessage("");
    try {
      const url = `http://localhost:3030/reservations/${reservationId}/program_plannings/${planningId}/programs/${programId}/close_excursions`;
  ...
  const handleSave = async (e) => {
    ...
    console.log(excursionsId);
    try {
      const response = await fetch(
        `http://localhost:3030/reservations/${reservationId}/program_plannings/${planningId}/excursions`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
