...
export default function ProgramsCircuit(props) {
  ...
  const fetchCircuit = async () => {
    ...
    setMessage("");
    try {
      const url = `http://localhost:3030/circuits/${circuitId}`;
      ...
      if (!response.ok) {
        setMessage("Failed to fetch circuit");
        return;
      }
      const data = await response.json();
      setCircuit(data);
      ...
      console.error("Error:", error);
      setMessage("Error fetching circuit");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <div className="card-header pb-0">
        <h6 style={{ textTransform: "uppercase" }}>{circuit.name}</h6>
