...
export default function FormTransfer(props) {
  const { title, method, departure, arrival, price, transferId, onCancel } =
    props;
  ...
  const handleSave = async (e) => {
    ...
    try {
      const idUrl = method === "PUT" ? `/${transferId}` : "";

      const response = await fetch(`http://localhost:3030/transfers${idUrl}`, {
        method: method,
        headers: {
  ...
  return (
    <div className="card p-4 shadow-lg rounded-3" style={{ width: "45%" }}>
      <div
